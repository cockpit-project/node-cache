...
const AccessConsoles = ({ children, textSelectConsoleType = 'Select console type', textSerialConsole = 'Serial console', textVncConsole = 'VNC console', textDesktopViewerConsole = 'Desktop viewer', preselectedType = null }) => {
    ...
    react_1.default.Children.toArray(children).forEach((child) => {
        if (isChildOfType(child, VNC_CONSOLE_TYPE)) {
            selectOptions.push(react_1.default.createElement(react_core_1.SelectOption, { key: VNC_CONSOLE_TYPE, id: VNC_CONSOLE_TYPE, value: { value: VNC_CONSOLE_TYPE, toString: () => textVncConsole } }));
        }
        else if (isChildOfType(child, SERIAL_CONSOLE_TYPE)) {
            selectOptions.push(react_1.default.createElement(react_core_1.SelectOption, { key: SERIAL_CONSOLE_TYPE, id: SERIAL_CONSOLE_TYPE, value: { value: SERIAL_CONSOLE_TYPE, toString: () => textSerialConsole } }));
        }
        else if (isChildOfType(child, DESKTOP_VIEWER_CONSOLE_TYPE)) {
            selectOptions.push(react_1.default.createElement(react_core_1.SelectOption, { key: DESKTOP_VIEWER_CONSOLE_TYPE, id: DESKTOP_VIEWER_CONSOLE_TYPE, value: { value: DESKTOP_VIEWER_CONSOLE_TYPE, toString: () => textDesktopViewerConsole } }));
        }
        else {
            const typeText = getChildTypeName(child);
            selectOptions.push(react_1.default.createElement(react_core_1.SelectOption, { key: typeText, id: typeText, value: { value: typeText, toString: () => typeText } }));
        }
    });
    return (react_1.default.createElement("div", { className: react_styles_1.css(AccessConsoles_1.default.console) },
        react_1.default.Children.toArray(children).length > 1 && (react_1.default.createElement("div", { className: react_styles_1.css(AccessConsoles_1.default.consoleActions) },
            react_1.default.createElement(react_core_1.Select, { "aria-label": textSelectConsoleType, placeholderText: textSelectConsoleType, toggleId: "pf-c-console__type-selector", variant: react_core_1.SelectVariant.single, onSelect: (_, selection, __) => {
                    setType(selection);
                    setIsOpen(false);
